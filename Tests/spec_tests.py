...
parser.add_argument("--fuzz-corpus", help="convert test cases to fuzz corpus")
...
def do_test(converter, test, normalize, result_counts):
    ...
    if retcode == 0:
        ...
        if passed:
            ...
            print_test_header(
                test["section"], test["example"], test["start_line"], test["end_line"]
            )
            out(test["markdown"] + "\n")
            if unicode_error:
...
if __name__ == "__main__":
    ...
    if args.dump_tests:
        ...
        converter = swift_cmark  # CMark(prog=args.program, library_dir=args.library_dir).to_html
